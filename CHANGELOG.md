...
### Fixed (Issue #96 — the landing quoted a currency Stripe never charged)
- **Every non-euro visitor was shown a price the checkout would not take, and three of the four were shown one lower than what they were about to pay** (`funnel/landing-shared/landing.js`). `Currency.detect()` guesses a currency from the browser timezone; `populate()` then read the amount from `/api/prices` and, when that price carried no entry for the guessed currency, fell back to the **base-currency amount** while `_fmt()` stamped the **requested symbol** onto it. No price in the live Stripe account has `currency_options` configured at all — `/api/prices` returns `base_currency: eur, currency_amounts: { eur: N }` for all five — so the fallback was not an edge case, it was every non-euro buyer: Australia quoted A$19.99 against a charge of €19.99 ≈ A$33.18 (**+66%**), Canada CA$19.99 vs ≈ CA$29.6 (**+48%**), the US $19.99 vs ≈ $21.8 (+9%), the UK £19.99 vs ≈ £17.3 (−13%). The same wrong pair went to Meta — `currency: 'AUD', value: 19.99` for a euro charge — so Ads Manager revenue and ROAS were off by the identical factor, and the two errors were invisible to each other because both sides agreed. A currency now counts as **live** only when *every* price the checkout can reach carries an explicit `currency_amounts` entry for it; anything short of that and the page quotes the base currency, symbol and amount together, which is exactly what Stripe will take. `detect()` keeps its old meaning — a guess about where the buyer is — and the new `effective()` is the currency that will actually be billed, the only one the page, the `create-checkout` payload or the pixel may report. The pixel's two hardcoded `usd` fallbacks went with it: `PRICES.usd` does not exist under a EUR-only account, so a conversion would have been reported as blank-value USD. **Nothing hardcodes EUR** — the moment `currency_options` are configured in Stripe, local pricing appears on its own with no further deploy, and a currency configured on some prices but not others still correctly falls back rather than half-working. New `currency.js` harness, 16 assertions across four timezones plus a simulated USD-configured account; 5 fail against the previous code.
- **Not fixed, and it is the other half of the same defect:** `create-checkout.js` normalises the incoming `currency` and returns it in the response, but **never passes it to `subscriptionSchedules.create()`**. Even with `currency_options` configured, the schedule would still bill in the price's base currency and the response field would stay decorative. Left deliberately to a follow-up that can be tested against a live Stripe key — shipping unverified payment-path code is what produced this class of bug. Closes #96.

### Changed (Issue #98 — create-checkout spent round trips it could predict)
- **`POST /api/create-checkout` measured ~3.2s against production, and a first-time buyer paid all of it** (`funnel/api/create-checkout.js`). `Checkout.prefetch()` can only warm an intent for a buyer whose address is already remembered, which by definition excludes everyone who has not bought before — they type their email and watch "Loading secure payment form…" for the full round trip. Three of the handler's Stripe calls were avoidable. A customer created one line above cannot have subscription schedules, so scanning for them asked Stripe a question whose answer was already known; skipped entirely for new customers. The cancel loop `await`ed each cancellation before starting the next, charging N round trips for N independent operations — a buyer who had compared a few tiers paid for every comparison. And the pixel-metadata refresh touches a different resource than the schedule scan with neither gating the other, so the two now overlap instead of queueing. Post-deploy production measurement: new customers ~3.17s → ~2.90s mean over three samples each; the returning-customer path is unchanged in the measurable case because a repeated call leaves at most one open schedule to cancel, so the parallel-cancel win only materialises for buyers who actually generate several. **`paymentIntents.update({ setup_future_usage })` was deliberately left alone** — folding it elsewhere risks both subscription renewals and the off-session upsell charge, and neither is verifiable without a live Stripe key. `schedules.create` → `finalizeInvoice` → `paymentIntents.update` are sequential by construction and are the ~2.7s floor the measurements show.
- **First backend test in the repo** (`funnel/tests/create-checkout.test.mjs` + `funnel/package.json` `test` script + `scripts/smoke-test.sh` check 6). No Stripe key exists in most environments, so the SDK is mocked via `node --experimental-test-module-mocks` and the **call sequence itself** is what gets asserted: which calls are made, in what order, and which overlap in wall-clock. 13 assertions — no schedule scan or metadata update for a new customer, a returning buyer's open schedules still cancelled while a `released` one is still left alone, cancels still completing before the new schedule opens, three cancels spanning one round trip rather than three, an unchanged response shape, `setup_future_usage` still applied, and the 400 guards intact. 3 fail against the previous handler. Check 6 skips with an `info` line when `funnel/node_modules` is absent, so the post-deploy hook stays green on machines that have not run `npm install`. Closes #98.

### Fixed (Issue #94 — landing checkout charged a truncated email address)
- **A buyer paying from either landing variant could be charged under a half-typed address and then locked out of the product they had just bought** (`funnel/landing-shared/landing.js`). `EMAIL_RE` (`^[^\s@]+@[^\s@]+\.[^\s@]+$`) matches long before the buyer is finished — for `oleksandr@gmail.com` it already matches at `oleksandr@gmail.c` — and `onEmailInput()` opened the PaymentIntent on that keystroke. Because `build()` holds `building` for the whole round trip (3–5s measured against production Vercel), the `!this.building` guard then swallowed the remaining `om`, and nothing re-read the field once the build settled. The Stripe customer, the subscription, the Supabase `subscriptions` row and the CAPI Purchase all carried the truncated address while `PostPay.start()` provisioned the Supabase account from the full one, so the webapp — which looks subscriptions up by email — found nothing for someone who had just paid. Support could not find the payment under the address the buyer gave them either, and the hashed `em` sent to Meta was of an address that exists nowhere. Secondary symptom: the button sat on "Preparing secure checkout…" for the full ~6.5s with no progress signal, which on its own read as a dead form. Three layers of fix: the input-driven build is debounced (650ms) and committed immediately on `blur`/`change`/submit so a buyer moving to the card field waits for nothing; `build()` re-reads the field when it settles and rebuilds if the address moved on, which is the only chance to catch keystrokes the in-flight guard dropped; and `pay()` now refuses to confirm an intent whose `mountedKey` does not match the live tier and address, whatever race produced the mismatch. `open()` routes through the new `commitEmail()` so a remembered address whose intent is still mounted skips a redundant round trip. **#91 could not have caught this**: funnel-v2 captures the email on its own screen and commits it with Continue, so the engine never sees a partial address, and every #91 test assigned `input.value` in one shot and fired a single `input` event — the intermediate states never existed. New regression tests type character by character at both fast and hunt-and-peck speeds and were verified to fail without the change.
- **The post-build recheck, in its first form, retried a failed checkout forever.** Comparing the settled field against `mountedKey` looked equivalent to comparing it against the address the build was for, but a failed `create-checkout` never sets `mountedKey` — so every failure re-entered `build()`, which cleared the error box and called the API again: 9 calls in the first 15 seconds of a repro and climbing, with the buyer parked on a spinner that never resolved or explained itself. The comparison is against the build's own address, which can only differ if the buyer actually typed something new. Caught on an adversarial pass over the fix itself, before merge; covered by a test asserting one attempt and a surviving error message. Closes #94.

### Added (Issue #91 — checkout parity: landing ↔ funnel-v2, plus a Meta Pixel audit)
- **Both landing variants now run funnel-v2's full three-step post-payment flow** (`funnel/landing-shared/landing.js` + `funnel/landing-quiz/index.html` + `funnel/landing-direct/index.html` + `funnel/landing-shared/core.css` + new `funnel/assets/upsell/{hero,coach,help,progress,showcase}.jpg`). Previously the landing took the buyer's money and redirected straight to the webapp on a provisioned session: no add-on offer, and no password. The second half of that mattered most — `provision-account.js` sets a random UUID it never returns, so a landing buyer could use the app until the handed-off session lapsed and then had no way back in. The flow is now `Stripe embedded Payment Element` → `AI Companion upsell` → `set password` → webapp, handed off through the same `#access_token`/`#refresh_token` URL hash (plus `&upsell=1`, consumed by `Login.tsx` per #79). New `PostPay` object owns steps 2–3; two new overlays (`#upsell` full-bleed, `#account`) are added to both variants' static HTML, deliberately without a close affordance — money has changed hands, so `Escape` and dismiss are disabled and every failure path still lands the buyer in the webapp with whatever session exists (declined or misconfigured add-on = silent skip; failed `create-user` = plain message then redirect on the provision session after 2.2s). Password validation mirrors `create-user.js:44` (both fields, match, ≥8) so the form can't bounce off a server 400. Upsell art re-encoded from funnel-v2's 11 MB of PNGs down to 502 KB of JPEG.
- **The upsell step is self-gating on infrastructure, not on a feature flag.** `/api/prices` omits the `upsell` key when `STRIPE_PRICE_UPSELL` is unset, so `Currency.upsellAvailable` skips step 2 entirely rather than advertising a €0.00 add-on whose CTA then 500s — and the step appears on its own the moment the env var is configured, with no code deploy. **`STRIPE_PRICE_UPSELL` is not set in production as of this entry, so the live landing flow is payment → password → webapp.**
- **PaymentIntent prefetch ported from funnel-v2** (`Checkout.prefetch()`): warmed when `#pricing` enters the viewport for a buyer whose address is remembered (`mc_landing_email_v1`), re-warmed on tier change, reused by `build()` so the Payment Element is mounted rather than loading when the modal opens.

### Fixed (Issue #91 — Meta Pixel completeness on the landing)
- **`ViewContent` + `AddToCart` now fire on offer view; they were absent from the landing entirely** (`funnel/landing-shared/landing.js`). Both fire together when `#pricing` scrolls into view, matching `engine/app.js:6081-6084` which fires them on paywall render — the equivalent funnel depth, so the two surfaces are comparable in Ads Manager. `ViewContent` used to fire on checkout-modal open, which is a strictly later step, and `AddToCart` never fired at all.
- **`Lead` moved from quiz completion to email entry, once per session.** `landing-direct` has no quiz, so it never reported a Lead and the two variants could not be compared against each other or against the funnel (`engine/app.js:4913` fires on email). Advanced matching now attaches the address properly via `fbq('init', pixelId, { em })`, which hashes in the browser. Deliberately **not** copied from the engine: its extra `em: <plaintext>` event parameter, which is not a matching channel and only puts a raw address in `custom_data`.
- **Server-side CAPI Purchase now carries browser identifiers and the real page URL** (`funnel/api/create-checkout.js` + `funnel/api/webhook.js`). The webhook runs from Stripe's request and has no cookies and no referrer of its own, so every landing conversion reached Meta with nothing but a hashed email and was reported under the funnel's hardcoded `event_source_url` — landing and funnel conversions were indistinguishable. `_fbp`/`_fbc`/`src_url` now ride to the server on the Stripe **customer's** metadata (the webhook already holds `invoice.customer`) and are read back by a shared `browserMeta()` helper used by both the subscription and AI Companion paths. `fbclid` is additionally persisted to `localStorage` and synthesized into `fb.1.<ts>.<id>` cookie format, because Meta writes `_fbc` only on the first pageview that carries the query param — a buyer who returns from a bookmark otherwise has the click id nowhere. Stripe **merges** a metadata update rather than replacing it, so all three keys are always written, empty string included; omitting one left a buyer who first arrived from an ad and later returned organically attributed to the stale click id.
- **Comparing tiers broke the payment outright** (`funnel/landing-shared/landing.js`). `create-checkout` cancels a customer's open subscription schedules before opening a new one, so every prefetch voids the PaymentIntent behind whatever is already mounted — and `prefetch()` skipped itself when the key matched the mounted one. `tier A → tier B → tier A` therefore left `open()` convinced its cached mount was still good, and the card declined on a schedule Stripe had already cancelled. Any new intent now clears `mountedKey` so the next open rebuilds. Caught on a second adversarial review pass, covered by a regression test verified to fail without the change.
- **Confirming the add-on left SKIP live**, so an impatient buyer could advance to the password step while their card was being charged and never learn they own the add-on. Both buttons now lock for the duration.

### Removed (Issue #91)
- **`CURRENCY_DISPLAY` and the `introDisplay`/`regularDisplay` response fields** (`funnel/api/create-checkout.js`) — a hardcoded €6.99/€14.99/€29.99 table that had drifted from live Stripe pricing and had zero consumers, verified by grep before deletion. `/api/prices` is the single source of truth. Closes #91.

### Added (Issue #87 — landing page as a second acquisition channel)
- **New direct-sale landing page at `funnel/landing-page/`** (`index.html` + `landing.css` + `landing.js` + `content.json`) — a bespoke, mobile-first single page that sells Mind Compass with an embedded Stripe checkout, no quiz. Sits alongside the funnel and reuses the same serverless backend end-to-end: `/api/create-checkout` (`{tierId,email,currency}` → `clientSecret`), `/api/prices` (live multi-currency, sessionStorage-cached with EUR fallback), and `/api/provision-account` on success, then the same cross-origin token-handoff to the webapp via URL hash. Same Stripe publishable key and Meta Pixel as the funnel — PageView (inline snippet) + ViewContent/InitiateCheckout on checkout open + Purchase on success, with `event_id = purchase_${subscriptionId}` so the browser event dedupes against the server CAPI event. `Currency`/`MetaPixel`/checkout logic are compact ports of `engine/app.js` (keep in sync); localhost `isDev()` dev-mock bypasses real charges. Copy is persuasion-first per owner directive — emotion + benefit + shame-reframe lead ("you're not broken; porn is a hijacked reward system"), with the neuroscience/CBT block kept as a short subordinate trust element (Voon 2014, WHO ICD-11 6C72, Antons 2022, Lally 2010) and a non-medical disclaimer. Data-driven via `content.json` (tiers, before/after, testimonials, FAQ, guarantee, legal ported from funnel-v2) mirroring the funnel's data/engine split. Gender-neutral voice throughout.
- **Landing page is now the homepage** (`funnel/vercel.json`): `/` redirects to `/landing-page/` (was `/funnel-v2/`), `/landing-page` normalized to `/landing-page/`; the funnel stays reachable at `/funnel-v2`. Landing footer legal links point directly at `/legal/*.html` (served by the existing global `/legal/:path+` rewrite) — no new per-path redirects.
- **Smoke test** (`scripts/smoke-test.sh`): added a landing-page health check (HTTP 200 + title contains "Mind Compass") to the Page Health section; runs automatically post-deploy. Closes #87.
...
### Changed (Issue #76 — follow-up² to #71: loosen coach length/validation, tighten safety to well-known crisis numbers)
- **Coach prompt loosened on length/validation, tightened on safety resource selection** (`webapp/prompts/aiCoach.ts`). Second follow-up to #71. Post-#75 production diagnostic confirmed 4 of 6 fixes landed cleanly (safety geo-awareness, sample-size calibration, formula-blocklist, hammer rule) but two were over-corrections — hard sentence caps and Validation prohibitions tuned to a tight-response preference that didn't match the user's actual preference for developed reflection (*"мені, як юзеру, хотілося б щоб відповіді були прям розгорнуті і класно описані"*) — plus one safety-mode hallucination where the coach gave a specific Ukrainian crisis line (`Teleffect`) that couldn't be externally verified. Three changes, all in `aiCoach.ts`, zero changes elsewhere. (1) Response shape limits: `hard caps with "cut, don't justify"` → `targets with "lean longer when depth serves the moment"`. Validation 1-2/cap 2 → 1-3/target with the explicit `Don't reframe / Don't ask a question / Don't offer a next step` prohibitions removed; shape renamed `Validation only` → `Validation-heavy` to signal acknowledgment-led rather than validation-only. Memorable rail `*A vent that gets fixed is a vent that gets ignored*` preserved but rearranged to FRAME the bullet rather than override the new permission, plus explicit `"fixing" here means pivoting to problem-solving or piling up an action list, not reflection itself` so a defensive model can't read the rail as forbidding what the new wording permits (peer-review catch). Reflection 2-4/last-is-question → 2-5/ending-with-question, with new positive trigger contrast `"when they're processing — not when they're venting raw"` to disambiguate from Validation-heavy now that both shapes can include a reflective question (peer-review catch). Reframe 4-6/cap 7 → 4-8/target. Urge mode (2-3 punchy) **UNCHANGED** — brevity here is clinically load-bearing, not a taste call; added `"Brevity here is clinically load-bearing — don't expand"` reinforcement to guard against the loosened-everywhere-else direction creeping in. (2) `# Safety` carve-out simplified: 4-clause enumerated lift list (`sentence caps don't apply; one-question cap doesn't apply; bullets-for-3+ doesn't apply`) → one explicit umbrella `"all formatting rules are lifted — length, one-question cap, bullets-for-3+, anything else"` with examples-not-enumeration, closing the same enumeration-as-exhaustive ambiguity peer-review flagged on #75's wording. (3) Crisis resource selection rewritten from `"if confident, name it"` to a BOTH-tests gate: (a) **well-known** — a typical adult in that country would recognize this number without context (e.g. `911` US/Canada, `112` EU/Ukraine, `999` UK/Ireland/Hong Kong, `000` Australia, `988` US suicide & crisis line, `116 123` Samaritans UK), AND (b) **confident currently active** — you know it hasn't been retired, rebranded, or replaced. Explicit DO-NOT list added: NGOs (`La Strada Ukraine`), obscure/regional services (`Teleffect`), smaller-market crisis short-codes — coach must fall back to generic if those are all it can recall. Section closes with `Bias toward generic if you have any doubt. Wrong resource info in a crisis is worse than generic guidance.` Preserves from #75: USER LOCATION pipeline, formula-blocklist + concrete alternative openers, sample-size calibration ("from this small window"), single-tool hammer rule, the global `drop all structure rules above` umbrella that opens `# Safety`. Out of scope (verified untouched): `coachContext.ts`, `AICoach.tsx`, `# What NOT to do` block, one-`?` primary rule in `# Formatting` (user accepted occasional 2-`?` slips as fine). `Closes #76`.

### Changed (Issue #73 — follow-up v3: Coach + Insights pills "breathing" halo)
- **Coach `Insights` + `New conversation` pills get a coloured glow halo on the same 2.4s beat as the existing colour pulse, plus organic hover/active feedback** (`webapp/src/index.css` + `webapp/components/AICoach.tsx` + `webapp/components/Dashboard.tsx`). The pulse keyframes (`@keyframes coach-reset-pulse`) used to animate only `color` + `border-color` — pills read as "okay, they change colour" but felt utilitarian. Three additions on the same beat: (1) `box-shadow` now pulses on the colour cycle — soft coloured drop-glow below the pill + a hairline outline tint just outside the border, gives the pill a "breathing" halo that follows the colour cycle without saturating the page; (2) blue midpoint glow is intentionally slightly wider + denser than the two endpoints (28px / 0.50 alpha vs 22px / 0.45 on purple + emerald) so the pulse subtly swells at the transition instead of reading as a flat 3-step loop; (3) hover lift via `translateY(-1px)` with a 180ms `cubic-bezier(0.22, 1, 0.36, 1)` ease + active settle via `scale(0.98)` at 80ms — organic physical feedback for the user's hand. Removed `shadow-sm` Tailwind utility from both pills (AICoach + Dashboard day-modal Insights button) because the keyframe now owns `box-shadow` — kept a base box-shadow + colour + border-color directly on `.animate-coach-reset-pulse` so `prefers-reduced-motion` users still see a purple-tinted pill with a soft halo instead of a browser-default grey one. `prefers-reduced-motion` cleanly disables the keyframe AND the hover/active transforms. Refs #73.

### Added (Issue #73 — follow-up v2: "Showing X of Y" range scope indicator)
- **Small gray scope hint under the time-range segmented control** (`webapp/components/Insights.tsx`). When a user's data clusters in the last few days (common — entries pile up around recent stress periods), switching `All-time` / `Last 30 days` / `Last 7 days` produced identical card numbers. Filter was correct, but with no visible signal that anything had changed, the UI read as "filter broken" — surfaced by `sukhomlyn95@gmail.com` whose 25 urge_log entries all fell within the last 5 days. Added an always-on one-liner: `Showing 25 of 25 urges · 5 of 7 check-ins` (counts swap with the active range). When windowed and full datasets overlap (X == Y), the user sees that no older data exists in this range. When they don't, they see the gap and understand the filter is doing real work. Refs #73.

### Changed (Issue #71 — follow-up: prompt hardening + locale-aware safety)
- **Coach prompt rules tightened across 5 dimensions, plus crisis-mode safety fix** (`webapp/prompts/aiCoach.ts` + `webapp/src/lib/coachContext.ts` + `webapp/components/AICoach.tsx`). Follow-up to #71 after a 10-prompt diagnostic conversation surfaced systemic discipline failures (length cap blown on Validation shape, multiple `?` per turn, "I hear you" formula leaks, unprompted N<10 pattern claims stated as fact, single-tool hammer repeating the same recommendation 4 turns in a row) plus a safety-critical bug: US-only crisis resources (`988`, `Crisis Text Line 741741`) were offered to a Ukrainian user who would have reached dead numbers in actual crisis. Response shape limits switched from char-counts (which LLMs can't reliably self-monitor) to sentence-counts with hard caps (Validation 1-2 / Reflection 2-4 / Reframe 4-6 cap 7 / Urge 2-3); Validation now explicitly forbids reframe + question + next-step ("a vent that gets fixed is a vent that gets ignored"); one-question cap strengthened from "AT MOST" to "Exactly ONE" with the rule echoed in both `# Formatting` and `# What NOT to do` for reinforcement; the negative-only "don't open with formula" rule replaced with concrete alternative openers (`"That's real."` / `"Okay — slow down."` / `"Yeah, that's a lot."` / `"Right — when you said X…"`); sample-size scope expanded from "absolute pattern claims" to ANY frequency/correlation/cause claim from <10 events stated as fact even when the user didn't ask, with Bad/Good examples; new single-tool hammer rule treats "text someone" / "call a friend" / "reach out" as semantically the same move (not just wording) and bans 3+ consecutive turns of the same underlying action. `# Safety` rewritten: drops ALL formatting/length rules in crisis mode (not just the question cap — sentence caps + bullets-only-for-3+ are also lifted so a real crisis response with resource + trusted person + step-away + safety check question doesn't get truncated), removes the hardcoded `988 in the US` example that the coach was treating as a default, and adds an "if uncertain about country OR number validity → DO NOT guess, use generic instructions" guardrail. The geo signal driving this: new `formatLocation()` formatter prepends `USER LOCATION (best-effort browser signal): timezone=Europe/Kyiv, language=uk-UA. Use timezone as primary geo signal; language is the UI preference and may not match physical location.` to the context block, sourced from `Intl.DateTimeFormat().resolvedOptions().timeZone` + `navigator.language` read at `handleSend` call time (so a traveling user gets locale-aware safety on the next turn). try/catch fallback matches the existing `webapp/components/ProGate.tsx:13-22` pattern; both values are whitespace-flattened + length-capped before interpolation (defense-in-depth against `Object.defineProperty(navigator, 'language', ...)` self-injection via DevTools — matches the `formatLetter` / `formatJournal` defang pattern). Wrapping header `USER CONTEXT (recent activity):` renamed to plain `USER CONTEXT:` since #71 already broadened the block beyond recent activity to include identity, plan status, and lifetime stats. The sample-size calibration win from #71 is preserved and reinforced — not weakened. No schema changes, no new endpoints, no DB columns. Manual post-deploy re-run of the 10-prompt diagnostic confirms regressions are gone. Refs #71.

### Changed (Issue #73 — follow-up v2: subtitle position + size)
- **Card subtitles moved above the metric and shrunk to `text-[11px]` so each fits on one line** (`webapp/components/Insights.tsx`). New reading order per card: header label → gray subtitle (right under label, indented `ml-9` to align with label text past the icon badge) → metric / chart / list. The eye now learns what the card means BEFORE parsing the number, not after. Subtitle copy tightened across all 8 cards so every one fits on a single line at the typical desktop (~390px content) and mobile (~340px content) card widths — e.g. "Top 3 actions ranked by how often they helped the urge pass." → "Ranked by how often they helped the urge pass." Refs #73.

### Added (Issue #73 — follow-up v2: Time range toggle)
- **Time range segmented control above the Insights cards** (`webapp/components/Insights.tsx`). Three pill buttons: `All-time` (default) / `Last 30 days` / `Last 7 days`. Active segment gets the white pill + shadow, inactives sit on the `bg-purple-100` track — visually mirrors the Coach `Insights` / `New conversation` button family. Switching the range filters the in-memory `urgeLog` + `checkIns` arrays via `useMemo` BEFORE the aggregation pass, so every card recomputes against the windowed dataset on the next tick. Zero extra Supabase round-trips — the full lifetime data is already loaded into App state for Coach context (#69, #71). Filter is sub-millisecond even at the practical N (hundreds of entries). Shorter windows naturally produce more "Need at least N sessions" placeholders because the `MIN_TRIES = 3` floor stays constant (kept on purpose: statistical significance matters more than card fullness; users who want richer data can switch back to All-time). Empty-state hero still gates on the UNFILTERED `urgeLog` so a user with history doesn't get demoted to the first-day motivational copy just because the current range happens to be empty. `TotalSurfsCard` subtitle "all-time" → "in this range" for accuracy across all three windows. Refs #73.

> **Deferred for now (Issue #73 v2 scope decision):** cross-device real-time sync via Supabase Realtime. Detailed rationale, implementation sketch, pricing analysis, and revisit triggers captured in `.claude/decisions/2026-06-15-realtime-cross-device-deferred.md`. TL;DR — no real user pain reported yet, real implementation cost is medium (dedup against optimistic updates, WebSocket reconnect reconciliation, cleanup on unmount/logout, soft 200-concurrent-connection ceiling on free tier as a new operational concern), and a cheaper polling-on-`visibilitychange` middle ground would likely cover 80 % of the perceived freshness for 10 % of the work. Triggers to reopen: ≥ 3 user complaints about stale cross-device data, DAU > 5 000, or any future feature whose value depends on second-by-second freshness (collaborative / social features, live human-therapist hand-off, etc).

### Changed (Issue #73 — follow-up: unified visual logic + mandatory subtitles)
- **Every Insights card now uses one palette and one layout** (`webapp/components/Insights.tsx`). Original PR shipped each card with its own colour (emerald / teal / sky / indigo / purple) to differentiate categories visually — read more like a kitchen-sink dashboard than a cohesive surface. Unified to a single purple palette (icons, headers, numbers, chart accents, list ranks, bar fills) so the grid reads as one coherent surface. Only intentional exception: the passed-vs-escalated split bar keeps semantic emerald + rose because chart colour there carries real meaning (good outcome / harder outcome). Removed the per-card `tintBg` / `tintIcon` props from `CardShell`; one badge style baked in.
- **Every card now carries a one-line gray subtitle that explains what it shows** (`webapp/components/Insights.tsx`). `CardShell.subtitle` is a required prop — pinned to the bottom of the card in `text-xs text-stone-500` so the eye lands on the metric first, then sees a plain-English explainer ("Top 3 actions ranked by how often they helped the urge pass", "Longest stretch of consecutive clean days on record", etc.). Resolves the "what does this card actually mean?" friction reported on first prod look.
- **Shared `BigMetric` primitive** (`webapp/components/Insights.tsx`) — three cards (Total surfs, Best streak, Resolution rate, Intensity trend) used near-identical `text-3xl md:text-4xl font-bold text-purple-900 tabular-nums` + small unit JSX. Extracted to one component so the typography hierarchy stays in lockstep if it ever changes.
- **Subtle copy + label tweaks**: "Triggers by time of day" → "When urges hit"; "Passed vs escalated" → "Resolution rate". Tighter, plainer. Confirmed metric correctness during this pass: `actionEffectivenessAll` (per-action success rate, `minTries=3` floor), `bestStreak` (consecutive CLEAN days, gap-breaks-run, SLIP wins same-day reduction), `timeOfDayBuckets` (Morning 5–12 / Afternoon 12–17 / Evening 17–22 / Late night 22–5 — matches `TIME_OF_DAY` constant), `monthlyIntensityTrend` (local-month grouping, skip zero-sample months). Real-time updates verified: `setUrgeLogEntries` / `setCheckIns` in `App.tsx` cascade through Insights `useMemo` deps `[urgeLog, checkIns]` so a freshly-completed urge or check-in appears immediately if the user is on the Insights page. Refs #73.

### Added (Issue #73)
- **Insights page — lifetime patterns & achievement cards** (new `webapp/components/Insights.tsx` + new `webapp/src/lib/insightsCalc.ts` + `webapp/types.ts` + `webapp/App.tsx` + `webapp/components/AICoach.tsx` + `webapp/components/Dashboard.tsx`). Paid-only data view that surfaces 8 lifetime aggregations over `urge_log` + `check_ins` so the user can see what's actually working for them over time. Cards: (1) Top 3 most-effective techniques by success rate (passed sessions / total uses), (2) Total surfs with passed/re-tried/escalated breakdown, (3) Best streak (longest consecutive CLEAN run), (4) Passed vs escalated split bar (excludes mid-session `still_here` from the ratio), (5) Triggers by time-of-day (4 buckets: Morning/Afternoon/Evening/Late night, matches existing `TIME_OF_DAY` constant), (6) Top 3 trigger feelings with horizontal proportion bars, (7) Monthly average-intensity trend as inline SVG polyline (skip months with zero samples so the chart doesn't render misleading zero-points), (8) Full action effectiveness ranking with `successes/tries · %` rows. All seven aggregation functions live in `insightsCalc.ts` as pure inputs-in / data-out helpers (no React, no I/O — unit-testable). `MIN_TRIES = 3` floor on action stats so a single 100%-success record can't dominate the top-3 list. Visualisations are inline SVG / CSS bars — zero new dependencies. New `View.INSIGHTS` enum + App-level routing with the existing ProGate pattern for free users (single Pro tier covers Insights + Coach + Urge Help). Two entry points: `Insights` button in the Coach hero action row (always visible, paired with `New conversation` when chat has real turns), and a smaller `Insights` button pinned right on the Clean / Slip status banner inside the Dashboard day-detail modal (closes the modal then navigates). Both buttons reuse `animate-coach-reset-pulse` so they read as the same affordance family. Empty state: `urgeLog.length === 0` → motivational hero + copy (no empty cards); per-card "Need at least N sessions" placeholder for partial data. Journal entries intentionally excluded — #66 plans to compress them into `coach_memory` prose rather than give them their own structured table, so they aren't a data source for analytics. Achievement-style cards use staggered `animate-in fade-in slide-in-from-bottom-2` (60ms × index) for a row-by-row reveal instead of one flash. Closes #73.

### Changed (Issue #71)
- **Coach `USER CONTEXT` block expanded with lifetime stats, explicit timeframes, sample-size calibration, and user's Future-Self Letter** (`webapp/src/lib/coachContext.ts` + `webapp/prompts/aiCoach.ts` + `webapp/services/claudeService.ts` + `webapp/App.tsx` + `webapp/components/AICoach.tsx`). Follow-up to #69 prompted by a real prod conversation where the coach said `"You've checked in 4 times total"` from a 7-day window snapshot and had no longer-horizon comparator. The block now renders 7 conditionally-rendered sections in identity-first order: `USER'S OWN WORDS (Future-Self Letter)` → `PLAN STATUS (Day N of 28)` → `LIFETIME (since YYYY-MM-DD, N days, with totals + current streak + last slip)` → `Check-ins (last 7 days)` → `Urges surfed (last 7 days)` → `Recent journal entries (most recent 10)` → `TODAY`. Section headers carry explicit timeframe labels so coach can never confuse a window for a total. Past Day 28 of the curriculum gets a `(plan complete)` annotation so coach can shift to maintenance-phase framing. `PRIOR CONVERSATION SUMMARY` header now annotated with `(last refreshed N days/weeks ago)` when older than 1 day so coach can deprioritize stale references. New `# How to read context` prompt section explains what each segment means, requires recent activity to win over stale memory note when sources conflict, and treats the user's letter as ground truth for values. Added sample-size calibration rule: no absolute pattern claims (`you always X`) from fewer than 10 events — use tentative framing instead. `getMemoryNote()` return type refactored to `{summary, updatedAt} | null` with one extra column added to existing `coach_memory` SELECT. Letter `values/identity/message` fields whitespace-flattened via the same defang pattern used for journal notes. Calendar-day diff via date-fns `differenceInCalendarDays` for plan day + lifetime days + last-slip days — matches user mental model better than 24h-bucket arithmetic. Cost: ~$0.0003 per coach reply added at Haiku 4.5 → ~$0.015/month per active user. No schema changes, no new endpoints. Closes #71.

### Changed (Issue #69)
- **Coach now sees a structured `USER CONTEXT (recent activity)` block on every reply** (`webapp/App.tsx` + `webapp/components/AICoach.tsx` + `webapp/prompts/aiCoach.ts` + new `webapp/src/lib/coachContext.ts`). Replaces the prior one-line `Date / Status / Emotions` summary in the system prompt with a 3-section block: last 7 days of check-ins (CLEAN/SLIP counts + slip days with trigger and time-of-day), last 7 days of urge_log (count, top feeling tally, avg intensity, escalated count), and the last 10 urge journal entries (raw trigger/intensity/note). Empty-state emits explicit `(new user — no activity yet)` so the coach gets a clear signal instead of an ambiguous empty section. New pure helper `buildCoachContext({ checkIns, urgeLog, journalEntries })` is unit-testable and runs entirely on the frontend — all data already lives in App state (`checkIns`, `urgeLogEntries` from #64) plus a sync `readJournal()` from localStorage; no server round-trips, no `/api/coach.js` changes. In-flight `urgeBlock` (when the user escalates from Help → Coach mid-session) is still prepended first so the current moment outranks long-term patterns. Sections cap at 10 entries each (≤1.5K input tokens added per call). Journal notes have whitespace collapsed (`\s+ → ' '`) to defang trivial prompt injection via fake "SYSTEM:" lines. Cost: ~$0.0006 per coach reply at Haiku 4.5 ($0.80/1M input) → ~$0.03/month per active user (50 replies/mo), ~$0.12/month per heavy user (200 replies/mo, bounded by existing quota cap). Supersedes the closed #66 — direct injection makes the journal useful as coach context without needing its own Supabase table or compression flow. Closes #69.

### Fixed (Issue #65)
- **Future-Self Letter no longer resets on a new device/browser** (`webapp/App.tsx` + `webapp/src/lib/urgeLog.ts` + `webapp/components/UrgeHelp.tsx` + `webapp/components/urgeActions/FutureSelfLetterScreen.tsx` + `webapp/components/urgeActions/index.ts` + new `supabase/migrations/20260614_future_self_letter.sql`). The Future-Self Letter (one of the 10 urge actions, category Reframe) previously lived in `localStorage` under `mc.future_self_letter.v1`, so opening the action on a new device showed the first-time guided write flow instead of the user's existing letter. Migrated to a new Supabase table `future_self_letter` (singleton per user, PK `user_id`). SQL column `letter_values` renamed from `values` to avoid the SQL reserved word; JS shape `{ values, identity, message, updatedAt }` preserved via `rowToLetter` / `letterToRow` mapping helpers. State lifted to App.tsx as `FutureSelfLetter | null | undefined` — the `undefined` sentinel is critical: it keeps the screen on a spinner during the initial fetch instead of flashing the first-time-write UI to a returning user. One-shot localStorage → Supabase migration in `loadUserData()` uses last-write-wins by `updatedAt` so a stale local letter can't overwrite a newer server one (the common case when the user wrote on device B after device A was already migrated). `FutureSelfLetterScreen` becomes prop-based; UrgeHelp special-cases its render branch with the letter props. `URGE_ACTION_SCREENS` registry now exports `GenericUrgeActionId = Exclude<UrgeActionId, 'future_self_letter'>` and excludes the letter screen (the other 9 still use the registry uniformly). Saves are optimistic + fire-and-forget (matches the `DailyCheckIn → check_ins` and #64 patterns). RLS policy `auth.uid() = user_id` (mirrors `urge_log` / `coach_memory`). Journal section of `urgeLog.ts` is intentionally untouched — issue #66 will fold it into the coach memory compression flow instead. Closes #65.

### Fixed (Issue #64)
- **`Urges Faced` count no longer resets on a new device/browser** (`webapp/App.tsx` + `webapp/src/lib/urgeLog.ts` + `webapp/components/Dashboard.tsx` + `webapp/components/UrgeHelp.tsx` + new `supabase/migrations/20260613_urge_log.sql`). The Help-tab urge log previously lived in `localStorage` under `mc.urge_log.v1`, so the Dashboard tile and `priorSurfCount` showed 0 on any new device the user signed in from. Migrated to a new Supabase table `urge_log` (composite PK `(user_id, id)` where `id` is the client-generated `Date.now()` Unix ms — makes one-shot migration idempotent and dedupes any cross-device collisions). State lifted from `urgeLog.ts` lib into `App.tsx` as `urgeLogEntries`, fetched in the existing `loadUserData()` parallel `Promise.all` alongside `check_ins`/`coach_messages`/`plan_progress`. Dashboard receives `urgesCount` as a prop; UrgeHelp receives `priorSurfCount` + `onAppendUrge(entry) → newTotal` so its celebration overlay still drives off the freshly-incremented total without a second read. Writes are optimistic + fire-and-forget (matches the `DailyCheckIn → check_ins` pattern). On first authed load post-deploy, any pending `mc.urge_log.v1` entries are bulk-upserted to Supabase, the local key is dropped, and the merged set (server ∪ local, deduped by `id`) is sorted by `endedAt`. RLS policy `auth.uid() = user_id` (mirrors `day_completions` / `coach_memory`). Letter (`mc.future_self_letter.v1`) and Journal (`mc.urge_journal.v1`) sections of `urgeLog.ts` are intentionally untouched — separate issues #65 and #66 handle them. Closes #64.
